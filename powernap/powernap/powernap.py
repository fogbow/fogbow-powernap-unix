...
from monitors import ProcessMonitor, LoadMonitor, InputMonitor, TCPMonitor, UDPMonitor, IOMonitor, WoLMonitor, ConsoleMonitor, PS2Monitor, DiskMonitor
...
class PowerNap:
    ...
    def set_default_values(self, var, value):
        ...
        if var == "recover_action":
            self.RECOVER_ACTION = eval(value)
            self.CUSTOM_RECOVER_ACTION = True
        if var == "warn":
